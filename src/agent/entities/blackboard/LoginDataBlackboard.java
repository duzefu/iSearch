...
public class LoginDataBlackboard {

	private static LoginDataBlackboard instance;
	private final static int MAX_INDEX=(1<<31)-1;
	private int mIndex;
	private Map<Integer, LoginData> mDataPool;
	private ReadWriteLock mLock;
	
	private LoginDataBlackboard(){
		mLock=new ReentrantReadWriteLock();
		mIndex=1;
		mDataPool=new HashMap<Integer, LoginData>();
	}
	
	private static LoginDataBlackboard getInstance(){
		
		if(null==instance){
			synchronized (LoginDataBlackboard.class) {
				if(null==instance) instance=new LoginDataBlackboard();
			}
		}
	...
	//LoginDataBlackboard的对象函数部分（非static，特别是注意线程同步的问题）列在这下面
	private LoginData get(int targetIndex){
	...
	private int addDataToPool(LoginData data){
	...
	public static LoginData getData(int targetIndex){
		return getInstance().get(targetIndex);
	}
	
	public static int addData(LoginData data){
